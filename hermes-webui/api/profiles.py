...
_PROFILE_DIRS = [
    ...
    'logs', 'plans', 'workspace', 'cron', 'webui_state',
]
...
def switch_profile(name: str, *, process_wide: bool = True) -> dict:
    ...
    # Always write the sticky active_profile file — even for per-client
    # cookie switches.  Without this, a browser close or cookie expiry
    # silently resets to 'default' because init_profile_state() on the next
    # server/container restart finds no active_profile file.
    try:
        ap_file = _DEFAULT_HERMES_HOME / 'active_profile'
